...
taking last year off. Back in 2019 [I used
...
Let's just dive in and we'll loop back to blog mechanics at the end.
...
- It's soothing convert a boolean to 0 or 1 with `Number()`
...
The worker is just a middleman between the component and the solution
...
And to close it out, debugging! If the calling component passes the `debug` prop
then the `dbg` function will post back messages. The component renders those
messages under the result, whenever it deigns to arrive:
